...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($orgao)) {
        ...
        try {
            $stmt = $db->prepare("UPDATE unidades SET orgao = ?, nome = ?, municipio = ?, uf = ? WHERE id = ?");
            $stmt->execute([$orgao, $nome, $municipio !== '' ? (string)$municipio : null, $uf ?: null, $id]);
...
$val = function($k, $d = '') use ($item) {
    return isset($_POST[$k]) ? $_POST[$k] : ($item[$k] ?? $d);
};
...
<div class="card">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">
            <i class="fas fa-edit mr-2"></i>Editar Unidade
        </h2>
    ...
    <form method="POST" class="space-y-6">
        <div class="form-group">
            ...
            <select id="orgao" name="orgao" class="form-control" required>
                ...
                <option value="<?php echo htmlspecialchars($org['id']); ?>" <?php echo $val('orgao') == $org['id'] ? 'selected' : ''; ?>>
        ...
        <div class="form-group">
            ...
            <input type="text" id="nome" name="nome" class="form-control" maxlength="255" value="<?php echo htmlspecialchars($val('nome')); ?>" required>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="form-group">
                ...
                <select id="uf" name="uf" class="form-control">
                    ...
                    <option value="<?php echo htmlspecialchars($est['sigla']); ?>" data-estado-id="<?php echo (int)$est['id']; ?>" <?php echo $val('uf') === $est['sigla'] ? 'selected' : ''; ?>>
            ...
            <div class="form-group">
                ...
                <div class="relative" id="municipio-combobox">
                    ...
                    <input type="hidden" id="municipio" name="municipio" value="<?php echo htmlspecialchars($val('municipio')); ?>">
...
(function() {
    ...
    const initMunicipioId = <?php echo json_encode($val('municipio')); ?>;
    ...
    if (initMunicipioId) {
        var m = municipios.find(function(x) { return x.id == initMunicipioId || x.id == parseInt(initMunicipioId, 10); });
        if (m) {
            municipioSearch.value = m.nome + ' (' + m.sigla + ')';
            municipioSearch.dataset.id = m.id;
        } else {
