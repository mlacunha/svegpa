...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($login) || empty($senha)) {
        ...
        $user = validateLogin($login, $senha, $dbError);
        if ($user) {
            $_SESSION['user'] = [
                'login' => $user['login'],
        ...
        $error = $dbError ? 'Erro de conexão com o banco. Verifique config/database.php e conectividade.' : 'Login ou senha incorretos. Verifique se a conta está ativa.';
    }
}
...
} elseif ($msg === 'link_invalido') {
    $msgText = 'Link de redefinição inválido. Solicite novamente em Esqueci a senha.';
} elseif ($msg === 'link_expirado') {
    $msgText = 'O link de redefinição expirou. Solicite um novo em Esqueci a senha.';
} elseif ($msg === 'senha_alterada') {
    $msgText = 'Senha alterada com sucesso! Faça login com a nova senha.';
}
?>
<!DOCTYPE html>
<html lang="pt-BR">
    ...
    <div class="w-full max-w-sm">
        <div class="bg-white rounded-lg shadow-lg p-8">
            <div class="text-center mb-6">
                ...
                <p class="text-sm text-gray-500">Coleta de Dados da Sanidade Vegetal</p>
            </div>
            ...
            <form method="POST" class="space-y-4">
                ...
                <p class="text-center mt-2">
                    <a href="<?php echo htmlspecialchars($base); ?>forgot_password.php" class="text-sm text-primary hover:underline">Esqueci a senha</a>
                </p>
            </form>
        </div>
