...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($orgao)) {
        ...
        try {
            $stmt = $db->prepare("UPDATE cargos SET orgao = :orgao, sigla = :sigla, nome = :nome WHERE id = :id");
...
<div class="card">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">
            <i class="fas fa-edit mr-2"></i>Editar Cargo
        </h2>
    ...
    <form method="POST" class="space-y-6">
        <div class="form-group">
            ...
            <select id="orgao" name="orgao" class="form-control" required>
                ...
                <option value="<?php echo htmlspecialchars($org['id']); ?>" <?php echo (isset($_POST['orgao']) ? $_POST['orgao'] : $cargo['orgao']) == $org['id'] ? 'selected' : ''; ?>>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="form-group">
                ...
                <input type="text" id="sigla" name="sigla" class="form-control" maxlength="50" value="<?php echo htmlspecialchars($_POST['sigla'] ?? $cargo['sigla']); ?>" required>
            </div>
            <div class="form-group">
                <label for="nome">Nome *</label>
                <input type="text" id="nome" name="nome" class="form-control" maxlength="255" value="<?php echo htmlspecialchars($_POST['nome'] ?? $cargo['nome']); ?>" required>
            </div>
        </div>
