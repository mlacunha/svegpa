...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nome)) {
        ...
        try {
            $stmt = $db->prepare("INSERT INTO produtores (id, n_cadastro, cpf_cnpj, RG_IE, nome, CEP, endereco, bairro, municipio, uf, telefone, email) VALUES (UUID(), :n_cadastro, :cpf_cnpj, :RG_IE, :nome, :CEP, :endereco, :bairro, :municipio, :uf, :telefone, :email)");
...
<div class="card">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">
            <i class="fas fa-plus-circle mr-2"></i>Novo Produtor
        </h2>
    ...
    <form method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-6">
            ...
            <div class="form-group md:col-span-1">
                ...
                <select id="uf" name="uf" class="form-control">
                    ...
                    <option value="<?php echo htmlspecialchars($e['sigla']); ?>" <?php echo (($_POST['uf'] ?? '') === $e['sigla']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($e['sigla'] . ' - ' . $e['nome']); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="form-group">
                ...
                <input type="text" id="telefone" name="telefone" class="form-control" maxlength="20" value="<?php echo htmlspecialchars($_POST['telefone'] ?? ''); ?>">
            </div>
            <div class="form-group">
                <label for="email">E-mail</label>
                <input type="email" id="email" name="email" class="form-control" maxlength="100" value="<?php echo htmlspecialchars($_POST['email'] ?? ''); ?>">
            </div>
        </div>
