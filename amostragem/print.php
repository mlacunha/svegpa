 ...
 * Recebe id (termo_inspecao). Logo do órgão no topo quando disponível.
 */
...
$stmt = $db->prepare("
    SELECT t.id, t.termo_coleta, t.data_amostragem, t.id_usuario, t.id_auxiliar, t.id_propriedade,
           ...
           o.nome as usuario_orgao_nome, o.sigla as usuario_orgao_sigla, o.logo as usuario_orgao_logo,
...
$logoOrgao = trim($t['usuario_orgao_logo'] ?? '');
$logoOrgaoUrl = $logoOrgao ? (getBasePath() . $logoOrgao) : '';
...
$narrativaAux = '';
if ($temAuxiliar) {
    $prefixo = $formacaoAux ? formacaoOuCargoComA($formacaoAux) . ' ' : '';
    $auxCompleto = $prefixo . $auxiliar;
    if ($cargoAux) $auxCompleto .= ', ' . formacaoOuCargoComA($cargoAux);
    if ($orgaoAux) $auxCompleto .= ' da ' . $orgaoAux . ($orgaoSiglaAux ? ' – ' . $orgaoSiglaAux : '');
    $narrativaAux = ' auxiliado pelo ' . $auxCompleto . ',';
}

$partesResp = array_filter(array_map('formacaoOuCargoComA', [$formacaoResp, $cargoResp]));
...
<head>
    ...
    <style>
        ...
        .header-logo { max-height: 80px; max-width: 100%; object-fit: contain; margin-bottom: 0.5em; }
        ...
        .assinatura-row { display: flex; justify-content: space-between; align-items: flex-start; gap: 0; }
        .assinatura-block { flex: 0 1 50%; min-width: 0; text-align: center; }
        .assinatura-block:only-child { flex: 0 1 50%; }
...
<body>
    ...
    <div class="header">
        <?php if ($logoOrgaoUrl): ?>
        <img src="<?php echo htmlspecialchars($logoOrgaoUrl); ?>" alt="Logo do órgão" class="header-logo">
        <?php endif; ?>
    ...
    <div class="assinatura">
        <div class="assinatura-row">
            <div class="assinatura-block assinatura-esq">
                <div class="assinatura-nome"><?php echo htmlspecialchars($responsavel ?: '_________________________'); ?></div>
                <div class="assinatura-info"><?php echo htmlspecialchars(formacaoOuCargoComA($formacaoResp)); ?></div>
                <div class="assinatura-info"><?php echo htmlspecialchars(formacaoOuCargoComA($cargoResp)); ?></div>
                <div class="assinatura-info">Carteira <?php echo htmlspecialchars($matriculaResp ?: '_________'); ?></div>
            </div>
            <?php if ($temAuxiliar): ?>
            <div class="assinatura-block assinatura-dir">
                <div class="assinatura-nome"><?php echo htmlspecialchars($auxiliar ?: '_________________________'); ?></div>
                <div class="assinatura-info"><?php echo htmlspecialchars(formacaoOuCargoComA($formacaoAux)); ?></div>
                <div class="assinatura-info"><?php echo htmlspecialchars(formacaoOuCargoComA($cargoAux)); ?></div>
